...
parser.add_argument('--epochs', type=int, default=10, help='training epochs')  # candidate: 20
parser.add_argument('--batch_size', type=int, default=64, help='training batch size')
parser.add_argument('--ec_kl_lambda', type=float, default=0.03, help='emotion and cause kl weight')
parser.add_argument('--label_smoothing', type=float, default=0.1, help='label smoothing')
parser.add_argument('--mmd_loss_weight', type=float, default=5, help='emotion multitask loss weight')  # candidate: 30
...
parser.add_argument('--self_iteration', type=int, default=50, help='self-training iteration')  # candidate: 50
...
log = open('memorization_drl_ec_mmd_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
for para in paras:
    print('mmd weight:', para)
    opt.mmd_loss_weight = para
    print('\n############ target domain {} ############\n'.format(targets_zh[0]))
    ...
    model.to(device)
    init_self_model = DrlClassifier(opt)
    init_self_model.to(device)
    ...
    test_path = "pair_data/emotion/" + targets_zh[0]
    ...
    for i in range(opt.self_iteration):
        ...
        if i == 0:
            fix_neg_indices = self_train_df.index[self_train_df['label'] == 0].tolist()
            fix_neg_pairs = self_train_df.iloc[fix_neg_indices]['pair'].tolist()

        # obtain positive samples
        cru_pos_indices = self_train_df.index[self_train_df['label'] == 1].tolist()
        cru_pos_pairs = self_train_df.iloc[cru_pos_indices]['pair'].tolist()

        if pre_pos_pairs:
            print('pre positive', pre_pos_pairs)
        print('cru positive', cru_pos_pairs)
        print('fix negative', fix_neg_pairs)

        if pre_pos_pairs:
            num_change = 0
            for old, new in zip(pre_pos_pairs, cru_pos_pairs):
                if old != new:
                    num_change += 1
            cha_percentage = num_change / len(cru_pos_indices)
            pos_changes.append(cha_percentage)
            print("positive sample changing percentage: {:.4f}\n".format(cha_percentage))
            pre_pos_pairs = cru_pos_pairs.copy()
        else:
            pos_changes.append(0.0)
            pre_pos_pairs = cru_pos_pairs.copy()

        # fix negative samples
        self_train_df.at[fix_neg_indices,'pair'] = fix_neg_pairs 
        ...
        #     self_best_trained_model = init_self_model
        ...
        precisions.append(self_p)
        recalls.append(self_r)
        f1s.append(self_f1)

    print("---running time: %s minutes ---" % ((time.time() - start_time) / 60))
    iters = list(range(1, opt.self_iteration + 1))
    plt.plot(iters, pos_changes, label="change of positive sample")
    plt.plot(iters, precisions, label="p")
    plt.plot(iters, recalls, label="r")
    plt.plot(iters, f1s, label="f1")
    plt.legend(loc='best')
    plt.savefig("memorization_fix_neg.png")
