...
log = open('zh_drl_ec_vi_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
class DrlClassifier(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # =============== Approximation network p(e|c) =============#
        self.ec_mu = nn.Sequential(nn.Linear(opt.ec_dim, opt.ec_dim),
                                   nn.ReLU(),
                                   nn.Linear(opt.ec_dim, opt.ec_dim))

        self.ec_log_var = nn.Sequential(nn.Linear(opt.ec_dim, opt.ec_dim),
                                        nn.ReLU(),
                                        nn.Linear(opt.ec_dim, opt.ec_dim),
                                        nn.Tanh())

        # =============== Classifier =============#
        self.emotion_classifier = nn.Linear(opt.ec_dim, 6)
    ...
    def forward(self, input_ids, att_masks, token_type_ids, emotion_labels, cause_labels, pair_labels, content_bow,
        ...
        # ============ Losses on approximation network ================#
        ec_aprx_loss = self.get_ec_aprx_loss(sampled_emotion_emb, sampled_cause_emb)
        ...
        vae_and_classifier_loss = opt.ec_mul_loss_weight * (emo_mul_loss + cau_mul_loss) + \
                                  opt.pair_mul_loss_weight * pair_mul_loss + \
                                  emotion_kl_loss + cause_kl_loss + \
                                  reconstruction_loss

        return sampled_emotion_emb, sampled_cause_emb, ec_aprx_loss, vae_and_classifier_loss
    ...
    def get_params(self):
        ...
        ec_aprx_params = list(self.ec_mu.parameters()) + list(self.ec_log_var.parameters())
        ...
        return ec_aprx_params, other_params
...
def train(train_loader, test_loader, model, optimizers, device, num_unpred_pairs, self_metrics=None, self_train=False):
    ...
    ec_aprx_opt, vae_and_cls_opt = optimizers
    ...
    for epoch in range(1, epochs + 1):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            e_embedding, c_embedding, ec_aprx_loss, vae_and_cls_loss = model(ids, att_masks, token_type_ids, emo_labels,
                                                                             cau_labels,
                                                                             labels, bow_reps,
                                                                             iteration)
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                ec_aprx_opt.zero_grad()
                ec_aprx_loss.backward(retain_graph=True)
                ec_aprx_opt.step()

                Rj_loss = model.get_ec_upper_loss(e_embedding, c_embedding)
                beta = (epoch - 1) * 0.1
                if beta > 1:
                    beta = 1
                vae_and_cls_loss += beta * Rj_loss
            ...
            # total loss
            training_loss += ec_aprx_loss.item() + vae_and_cls_loss.item()

            running_loss += ec_aprx_loss.item() + vae_and_cls_loss.item()
...
for domain in targets_zh:
    ...
    model.to(device)
    ec_aprx_params, vae_and_classifier_params = model.get_params()

    # define optimizers
    ec_aprx_opt = torch.optim.Adam(ec_aprx_params, lr=opt.aprx_lr)
    vae_and_cls_opt = torch.optim.Adam(vae_and_classifier_params, lr=opt.vae_lr)
    opts = [ec_aprx_opt, vae_and_cls_opt]

    # load data
    train_path = "domains/THUCTC_multiple/society_num.txt"
