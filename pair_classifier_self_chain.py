...
class ECPEDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        emo_sentence = pair.split("[SEP]")[0]
        cau_sentence = pair.split("[SEP]")[1]

        # v2
        inputs = self.tokenizer(emo_sentence, cau_sentence,
...
def read_ECPE_data(file_path, test=False):
    ...
    docs_pair_size = []
    if test:
        self_chain_docs = get_self_chain_docs(file_path)
    ...
    while True:
        ...
        if re.search("[0-9]{1,4}[\s][0-9]{1,2}", line):
            ...
            # keep docs that contain self-chain structure
            if test:
                if doc_id not in self_chain_docs:
...
def read_ECPE_data_pred(file_path, test=False):
    data_file = open(file_path, encoding="utf8")
    df = pd.DataFrame(columns=["pair", "label"])
    docs_pair_size = []
...
def get_self_chain_docs(file_path):
    data_file = open(file_path, encoding="utf8")
    self_chain_docs = []
    count_s = 0
    pair_count = 0
    while True:
        ...
        if re.search("[0-9]{1,4}[\s][0-9]{1,2}", line):
            ...
            for e, c in zip(emotions, causes):
                if e == c:
                    print("Self chain:", "doc_id:", doc_id, doc_len, "emotion:", emotions, "cause:", causes)
                    count_s += 1
                    self_chain_docs.append(doc_id)

    return self_chain_docs


# log
timestr = time.strftime("%Y%m%d-%H%M%S")
log = open('pair_classifier_self_chain_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
test_path = "domains/THUCTC_multiple/education.txt"
# test_path = "pair_data/emotion/finance.txt"
# test_path = "test.txt"
train_df, _ = read_ECPE_data(train_path)
# train_df, _, _ = read_ECPE_data_pred(train_path)
test_df, test_docs_pair_size = read_ECPE_data(test_path, test=True)
# test_df, test_docs_pair_size, num_unpred = read_ECPE_data_pred(test_path, test=True)
