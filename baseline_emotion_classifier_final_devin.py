...
final version of emotion classifier
"""
import argparse
import torch
import torch.nn
...
##### input
parser.add_argument('--language', default='en', help='zh and en')
parser.add_argument('--source_domain', default='enecpe_num', help='zh-society and home, en-enecpe_num, reccon_num')
parser.add_argument('--target_domain', default='reccon_test', help='zh-finance, society, education, entertainment and home, en-reccon_test and enecpe_num')
...
##### traing
parser.add_argument('--training_epoch', type=int, default=10, help='number of train iteration')
parser.add_argument('--self_epoch', type=int, default=5, help='number of self train epoch')
...
parser.add_argument('--bow_optimize', type=str, default='false', help='optimize the bow representation for English dataset')
...
if opt.language == 'zh':
    path = 'emotion_logs/zh_final_emotion_extraction_log_' + timestr + '.txt'
elif opt.language == 'en':
    path = 'emotion_logs/en_final_emotion_extraction_log_' + timestr + '.txt'
...
def print_training_info():
    print('\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>TRAINING INFO:\n')
    print('batch-{}, lr-{}, kb_softmax-{}, l2_reg-{}, confidence_threshold--{}'.format(
        opt.batch_size, opt.learning_rate, opt.keep_softmax, opt.l2_reg, opt.threshold))

    print('training_epoch-{}'.format(opt.training_epoch))
    print('self-training_epoch-{}'.format(opt.self_epoch))
    print('source_domain-{}, target_domain--{}\n'.format(opt.source_domain, opt.target_domain))
    print('bow_optimize-{}'.format(opt.bow_optimize))
...
def generate_self_train_data(source_data, best_pred):
    ...
    self_train = dir + opt.target_domain + ".txt"
    ...
    return self_train_loader, len(conc_data)
    ...
    def __init__(self, input_file, max_doc_len=75, max_sen_len=60, test=False):
        ...
        while True:
            ...
            for i in range(d_len):
                ...
                if emotion in ['0', '1', '2', '3', '4', '5', '6']:
                    y_emotion_tmp[i][int(emotion)] = 1
                ...
                # The emotion that this cause leads to.
                cause = current_line[2]
                if cause in ['0', '1', '2', '3', '4', '5', '6']:
                    y_cause_tmp[i][int(cause)] = 1
                else:
                    if cause == "happiness":
                        y_cause_tmp[i][0] = 1
                    elif cause == "sadness":
                ...
                if opt.language == 'en' and opt.bow_optimize == 'true':
                    sentence = current_line[-1].strip()
                else:
                    sentence = current_line[-1].strip().replace(" ", "")
...
class biLSTM(torch.nn.Module):
    def __init__(self, keep_softmax, input_size=opt.n_hidden * 2, hidden_size=opt.n_hidden, bert_dim=opt.bert_dim):
        ...
        self.senlayer = nn.Linear(bert_dim, 2 * opt.n_hidden, bias=True)
    ...
    def forward(self, x_ids, x_masks, x_types):
        ...
        # 150 * 7
        pred_emotion = F.softmax(self.nnlayer_pos(s1), dim=1)
        # 2 * 75 * 7
...
def train():
    ...
    save_dir = 'pair_data/predicted_emotion/source_{}/'.format(opt.source_domain)
    ...
    train = dir + opt.source_domain + ".txt"
    self_train = dir + opt.target_domain + ".txt"
    test = dir + opt.target_domain + ".txt"
    ...
    for i in range(1, opt.training_epoch + 1):
    ...
    num_iter = 1
    last_self_train_data_size = 0
    while True:
        self_best_model = load_ckp(os.path.join('ECPE_model/best_emotion_model', model_id + ".pt"), self_model)
        self_train_loader, self_train_data_size = generate_self_train_data(ECPE_data['train'],
                                                                           self_best_test_pred_emotion)

        if self_train_data_size > last_self_train_data_size:
            print('current self-training dataset size increment: {} --> {}'.format(last_self_train_data_size,
                                                                                   self_train_data_size))
            last_self_train_data_size = self_train_data_size
        else:
            break

        for j in range(1, opt.self_epoch + 1):
            start_time, step = time.time(), 1
            ...
            with torch.no_grad():
                ...
                if f1 > max_self_f1_emotion:
                    ...
                    # Using the predicted emotions as the input in the cause-emotion relation test dataset.
                    generate_pair_data(save_dir + opt.target_domain + ".txt", ECPE_data['test'].doc_id,
                                       ECPE_data['test'].doc_len,
                                       ECPE_data['test'].y_pairs, all_test_pred_cause_op.numpy(),
                                       all_test_pred_emotion_op.numpy(), ECPE_data['test'].x_ids)
                    print('save the predicted emotions......')
