...
log = open('drl_ec_mmd_self_chain_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
def read_ECPE_self_chain_data(file_path, test=False):
    data_file = open(file_path, encoding="utf8")
    df = pd.DataFrame(columns=["pair", "label"])
    docs_pair_size = []
    if test:
...
for domain in targets_zh:
    ...
    train_df, _ = read_ECPE_self_chain_data(train_path)
    test_df, test_docs_pair_size = read_ECPE_self_chain_data(test_path, test=True)
