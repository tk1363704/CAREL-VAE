...
log = open('drl_bow_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
class DrlClassifier(nn.Module):
    ...
    def forward(self, input_ids, att_masks, token_type_ids, emotion_labels, cause_labels, pair_labels, content_bow,
        ...
        # constrain bow for emotion-cause and content respectively
        con_weights = nn.Sigmoid()(self.content_classifier(self.dropout(sampled_content_emb)))
        ec_weights = 1 - con_weights
        # Discriminator Loss
        content_disc_preds_emo = self.get_content_disc_preds(sampled_emotion_emb)
        content_disc_loss_emo = self.get_content_disc_loss(content_disc_preds_emo, content_bow, ec_weights)
        content_disc_preds_cau = self.get_content_disc_preds(sampled_cause_emb)
        content_disc_loss_cau = self.get_content_disc_loss(content_disc_preds_cau, content_bow, ec_weights)
        # adversarial entropy
        ...
        # Multitask loss
        content_mul_loss = self.get_content_mul_loss(sampled_content_emb, content_bow, con_weights)
    ...
    def get_content_disc_loss(self, content_disc_preds, content_bow, weights):
        """
        ...
        content_disc_loss = nn.BCELoss(weight=weights.detach())(content_disc_preds, smoothed_content_bow)
    ...
    def get_content_mul_loss(self, content_emb, content_bow, weights):
        """
        ...
        # calculate cross entropy loss
        content_mul_loss = nn.BCELoss(weight=weights.detach())(preds, smoothed_content_bow)
