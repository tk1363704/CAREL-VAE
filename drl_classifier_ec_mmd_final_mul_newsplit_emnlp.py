...
Using newly split training / test dataset to do the experiments.
"""
import re
import pandas as pd
import random
...
from bow_util import get_bow_zh, get_bow_en, bow_tokenize
...
parser.add_argument('--bow_optimize', type=str, default='true', help='optimize the bow representation for English dataset')
parser.add_argument('--source_domain', default='home', help='zh-home, society, en-enecpe_num, and reccon_num')
parser.add_argument('--target_domain', default='education',
                    help='zh-society, finance, education and entertainment, en-reccon_test, history_num, war_new, romance, adventure_new, biography, enecpe_num')
...
parser.add_argument('--self_strategy', type=str, default='temporal_order_modification', help='self-training strategy, random or temporal_order, temporal_order_modification (taking causal orders into consideration)')
parser.add_argument('--adapter', default='false', help='using adapter or not, false refer to not using adapter, '
                                                        'otherwise using raw, sparsemax or entmax')
parser.add_argument('--head_number', type=int, default=4, help='number of multi heads for the adapters')
parser.add_argument('--confounding', type=str, default='false', help='using confounding mode in the model')
parser.add_argument('--predicted_emotion', type=str, default='true', help='using the predicted emotions stored in pair_data/predicted_emotion/source folder when testing if the value is true')
parser.add_argument('--round_up', type=str, default='true', help='using the round up for model.get_pair_preds')
...
if opt.language == 'zh':
    ...
    opt.bow_file = 'data/ecpe_and_reccon_all_data_pair_en.txt'
    if opt.bow_optimize == 'true':
        print('Use the optimized bow for the English dataset!')
        bow = get_bow_en(opt.bow_file, bow_optimize=True, tokenizer=tokenizer)
    else:
        print('Use the original bow for the English dataset!')
        bow = get_bow_en(opt.bow_file)
opt.pair_bow_dim = len(bow)

timestr = time.strftime("%Y%m%d-%H%M%S")
if opt.language == 'zh':
    path = 'result_logs/zh_final_cause_pair_extraction_mul_log_' + timestr + '.txt'
elif opt.language == 'en':
    path = 'result_logs/en_final_cause_pair_extraction_mul_log_' + timestr + '.txt'
...
if opt.confounding == 'true':
    print('Confounding mode is on...')
else:
    print('Confounding mode is off...')
...
class ECPEDataset(torch.utils.data.Dataset):
    def __init__(self, df):
        ...
        self.bow_optimize = True if opt.bow_optimize == 'true' else False
    ...
    def _get_bow_representations(self, text_pair):
        if self.language == 'zh' or (self.language == 'en' and self.bow_optimize is False):
            ...
            sen_words = bow_tokenize(text_pair, self.tokenizer)
...
class SparsemaxMultiheadAttention(nn.MultiheadAttention):
    ...
    def _scaled_dot_product_attention(self, q, k, v, attn_mask=None):
        ...
        k = k.transpose(-2, -1)  # Shape (B, H, S) -> (B, S, H)
        scores = q.matmul(k) / (self.embed_dim ** 0.5)
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None, average_attn_weights=True):
        # Compute the attention scores using the original mechanism
        q = self.q_proj(query)
        k = self.k_proj(key)
        v = self.v_proj(value)
        attn_output_scores, attn_output_weights = self._scaled_dot_product_attention(q, k, v, attn_mask)

        # Apply sparsemax to the attention weights to induce sparsity
        if need_weights:
            attn_output_scores = attn_output_scores.transpose(0, 1) # Shape (B, S, H) -> (S, B, H)
            attn_output_scores_shape = attn_output_scores.shape
            attn_output_scores = torch.reshape(attn_output_scores, (-1, attn_output_scores.shape[-1])) # Shape (S*B, H)
            sparsemax = Sparsemax(dim=1)
            sparsemax_output = sparsemax(attn_output_scores)
            sparsemax_output = torch.reshape(sparsemax_output, attn_output_scores_shape) # Shape (S, B, H)
            attn_output_weights = sparsemax_output.transpose(0, 1) # Shape (S, B, H) -> (B, S, H)
...
class EntmaxMultiheadAttention(nn.MultiheadAttention):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.q_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=True)
        self.k_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=True)
    ...
    def _scaled_dot_product_attention(self, q, k, attn_mask=None):
        # Compute the attention scores
        k = k.transpose(-2, -1)  # Shape (B, H, S) -> (B, S, H)
        scores = q.matmul(k) / (self.embed_dim ** 0.5)
        if attn_mask is not None:
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None, average_attn_weights=True):
        ...
        k = self.k_proj(key)
        v = self.v_proj(value)
        attn_output_scores, attn_output_weights = self._scaled_dot_product_attention(q, k, attn_mask)
        ...
        if need_weights:
            ...
            ent_attn_probs = entmax15(attn_output_scores, dim=-1)
            # # Apply dropout
            # ent_attn_probs = self.dropout(ent_attn_probs)
            ent_output = torch.reshape(ent_attn_probs, attn_output_scores_shape) # Shape (S, B, H)
            attn_output_weights = ent_output.transpose(0, 1) # Shape (S, B, H) -> (B, S, H)
...
class DrlClassifier(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.adapter != 'false':
            # emotion adapter
            self.emotion_q = torch.randn(1, 1, opt.bert_dim, requires_grad=True).to(device)
            if opt.adapter == 'raw':
                self.emotion_adapter = nn.MultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True)
            elif opt.adapter == 'sparsemax':
                self.emotion_adapter = SparsemaxMultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True)
            elif opt.adapter == 'entmax':
                self.emotion_adapter = EntmaxMultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True, dropout=0.0)

            # cause adapter
            self.cause_q = torch.randn(1, 1, opt.bert_dim, requires_grad=True).to(device)
            if opt.adapter == 'raw':
                self.cause_adapter = nn.MultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True)
            elif opt.adapter == 'sparsemax':
                self.cause_adapter = SparsemaxMultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True)
            elif opt.adapter == 'entmax':
                self.cause_adapter = EntmaxMultiheadAttention(opt.bert_dim, opt.head_number, batch_first=True, dropout=0.0)
    ...
    def forward(self, input_ids, att_masks, token_type_ids, emotion_labels, cause_labels, pair_labels, content_bow,
        ...
        # sentence embeddings
        # input_ids -> (64, 128)
        # pooler_output: last layer hidden-state of the first token of the sequence -> (64, 768)
        # last_hidden_state: sequence of hidden-states at the output of the last layer of the model -> (64, 128, 768)
        # attn_output -> (64, 1, 768)
        ...
        if opt.adapter != 'false':
            ...
            expanded_emotion_q = self.emotion_q.expand(input_ids.shape[0], self.emotion_q.shape[1], self.emotion_q.shape[-1])
            emotion_attn_output, _ = self.emotion_adapter(expanded_emotion_q, sentence_emb, sentence_emb)
            emotion_attn_output = torch.squeeze(emotion_attn_output, 1)

            expanded_cause_q = self.cause_q.expand(input_ids.shape[0], self.cause_q.shape[1], self.cause_q.shape[-1])
            cause_attn_output, _ = self.cause_adapter(expanded_cause_q, sentence_emb, sentence_emb)
            cause_attn_output = torch.squeeze(cause_attn_output, 1)
    ...
    def get_pair_preds(self, input_ids, att_masks, token_type_ids, round=True):
        if opt.adapter != 'false':
            sentence_emb = self.encoder(
                input_ids,
                attention_mask=att_masks,
            ...
            expanded_emotion_q = self.emotion_q.expand(input_ids.shape[0], self.emotion_q.shape[1],
                                                       self.emotion_q.shape[-1])
            emotion_attn_output, _ = self.emotion_adapter(expanded_emotion_q, sentence_emb, sentence_emb)
            emotion_attn_output = torch.squeeze(emotion_attn_output, 1)
            ...
            emotion_emb_mu, emotion_emb_log_var = self.get_emotion_emb(sentence_emb)
            cause_emb_mu, cause_emb_log_var = self.get_cause_emb(sentence_emb)
        ...
        return nn.Sigmoid()(self.pair_classifier(pair_emb)).cpu().detach().numpy().round().tolist() if round is True else nn.Sigmoid()(self.pair_classifier(pair_emb)).cpu().detach().numpy().tolist()
...
def read_ECPE_data(file_path, test=False):
    ...
    df = pd.DataFrame(columns=["pair", "label", "emotion", "temporal_order"])
    df["temporal_order"] = df["temporal_order"].astype(bool)
    docs_pair_size = []
        ...
        if re.search("[0-9]{1,4}[\s][0-9]{1,2}", line):
            ...
            if not test:
                ...
                for i, e in enumerate(true_emotions):
                    # true label has emotion but the predicted label doesn't, the cause-emotion pair must have emotions
                    if e not in pred_emotions and e != pre_e:
                        num_unpred_emotions += 1
                    # in case of two events cause a same emotion, for instance (6, 5), (6, 6)
                    elif e == pre_e:
                        ...
                        pre_e = e
                # use the emotions that appear in both the predicted and true labels to construct the positive pairs for target domain
            ...
            # test
                ...
                sen_ids = [i + 1 for i in range(doc_len)]
                # the emotions that found in the predicted label but not in the true label
            ...
            for pos_p in pos_pairs:
                ...
                # todo add an extra dimension as context data (separated by [SEP])
                # todo: Dose eng replace " " as "" make any sense?
                if opt.language == 'en' and opt.bow_optimize == 'true':
                    pos_sen_pair = sentence_list[emo_sen_id - 1].strip().split(",")[3] + " [SEP] " + \
                                   sentence_list[cau_sen_id - 1].strip().split(",")[3]
                else:
                ...
                temporal_order = True if cau_sen_id <= emo_sen_id else False
                new_row = {'pair': pos_sen_pair, 'label': 1, 'emotion': emotion, 'temporal_order': temporal_order}
            ...
            if neg_pairs:
                for neg_p in neg_pairs:
                    ...
                    # todo: Dose eng replace " " as "" make any sense?
                    if opt.language == 'en' and opt.bow_optimize == 'true':
                        neg_sen_pair = sentence_list[emo_sen_id - 1].strip().split(",")[3] + " [SEP] " + \
                        sentence_list[cau_sen_id - 1].strip().split(",")[3]
                    else:
                    ...
                    temporal_order = True if cau_sen_id <= emo_sen_id else False
                    new_row = {'pair': neg_sen_pair, 'label': 0, 'emotion': emotion, 'temporal_order': temporal_order}
                    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
...
def generate_self_train_data(test_docs_pair_size, test_df, test_loader, model, strategy, self_training_iteration, r_flag):
    # obtain prediction
    predicted_df = test_df.copy()
    model.eval()
    round_flag = True if r_flag == 'true' else False
    print("############ Iteration threshold: {} ############".format(1))
    if strategy == "temporal_order_modification" and self_training_iteration < 1:
        round_flag = False
        print("############ temporal_order_modification use round: {} ############".format(round_flag))
    with torch.no_grad():
        for batch_index, data in enumerate(test_loader):
            ...
            final_outputs = model.get_pair_preds(ids, att_masks, token_type_ids, round_flag)
    ...
    for doc_pair_size in test_docs_pair_size:
        ...
        if strategy == "temporal_order_modification":
            if self_training_iteration >= 1:
                strategy = "random"
            else:
                strategy = "temporal_order"
        ...
        for i in range(doc_pair_size):
            ...
            elif strategy == 'temporal_order':
                prob_dict[index] = prob

        if strategy == 'temporal_order':
            sort_prob_dict = sorted(prob_dict.items(), key=lambda x: x[1], reverse=True)
            index_ = None
            for i, kv in enumerate(sort_prob_dict):
                # Not following order.
                index_ = i
                key = kv[0]
                if not predicted_df.iloc[key]["temporal_order"]:
                    continue
                else:
                    pos_pair = predicted_df.iloc[key]["pair"]
                    pos_emotion = predicted_df.iloc[key]["emotion"]
                    break
            if index_ is not None and index_ < len(sort_prob_dict) - 1:
                neg_ran_index = randint(index_ + 1, len(sort_prob_dict) - 1)
                neg_pair = predicted_df.iloc[sort_prob_dict[neg_ran_index][0]]["pair"]
                neg_emotion = predicted_df.iloc[sort_prob_dict[neg_ran_index][0]]["emotion"]
            # else:
            #     print('no positive or negative pairs!')
        ...
        if pos_pair is not None and neg_pair is not None:
            ...
            df = pd.concat([df, pd.DataFrame({'pair': [pos_pair], 'label': [1], 'emotion': [pos_emotion]})],
                           ignore_index=True)
            df = pd.concat([df, pd.DataFrame({'pair': [neg_pair], 'label': [0], 'emotion': [neg_emotion]})],
                           ignore_index=True)
...
# language
if opt.language == 'zh':
    dir = "data/ECPE_new_dataset/"
elif opt.language == 'en':
    dir = "domains/Englishnovel_multiple/"

# load data
train_path = dir + opt.source_domain + ".txt"
print("############ Training dataset is {} ############".format(train_path))
if opt.language == 'zh':
    if opt.predicted_emotion == 'true':
        test_path = "pair_data/predicted_emotion/source_{}/".format(opt.source_domain) + opt.target_domain + ".txt"
    else:
        test_path = "data/ECPE_new_dataset/" + opt.target_domain + "_test.txt"
else:
    if opt.predicted_emotion == 'false':
        if opt.bow_optimize == 'true':
            test_path = "pair_data/emotion/" + opt.target_domain + "_optimize.txt"
        else:
            test_path = "pair_data/emotion/" + opt.target_domain + ".txt"
    else:
        test_path = "pair_data/predicted_emotion/source_{}/".format(opt.source_domain) + opt.target_domain + ".txt"
print("############ Test dataset is {} ############".format(test_path))
...
# best_trained_model = load_ckp(os.path.join(opt.best_model_path, "hometoentertainment_pretraining.pt"), model)

# self-training
print("############ Self-training Start ############")
print('Round mode is {}!'.format(opt.round_up))
...
for i in range(opt.self_iteration):
    ...
    self_train_df = generate_self_train_data(test_docs_pair_size, test_df, test_data_loader,
                                             ...
                                             strategy=opt.self_strategy, self_training_iteration=i, r_flag=opt.round_up)
    self_train_dataset = ECPEDataset(self_train_df)
    if len(self_train_dataset) == 0:
        print("No Training data samples!!!")
        continue
