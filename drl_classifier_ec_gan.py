...
parser.add_argument('--ecce_adv_loss_weight', type=float, default=1, help='emotion and cause adversary loss weight')
...
parser.add_argument('--adv_lr', type=float, default=0.003, help='adversary learning rate')
...
log = open('drl_ec_gan_log_' + timestr + '.txt', 'w', buffering=1)
sys.stdout = log
sys.stderr = log
...
class DrlClassifier(nn.Module):
    ...
    def forward(self, input_ids, att_masks, token_type_ids, emotion_labels, cause_labels, pair_labels, content_bow,
        ...
        vae_and_classifier_loss = opt.ecce_adv_loss_weight * (ec_entropy_loss + ce_entropy_loss) + \
                                  opt.ec_mul_loss_weight * (emo_mul_loss + cau_mul_loss) + \
                                  opt.pair_mul_loss_weight * pair_mul_loss + \
                                  emotion_kl_loss + cause_kl_loss + \
                                  reconstruction_loss

        return ec_disc_loss, ce_disc_loss, vae_and_classifier_loss
    ...
    def get_params(self):
        ...
        return ec_disc_params, ce_disc_params, other_params
...
def train(train_loader, test_loader, model, optimizers, device, num_unpred_pairs, self_metrics=None, self_train=False):
    ...
    ec_disc_opt, ce_disc_opt, vae_and_cls_opt = optimizers
    ...
    for epoch in range(1, epochs + 1):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            ec_disc_loss, ce_disc_loss, vae_and_cls_loss = model(ids, att_masks, token_type_ids, emo_labels, cau_labels,
                                                                 labels, bow_reps,
                                                                 iteration)
            ...
            # total loss
            training_loss += ec_disc_loss.item() + \
                             + ce_disc_loss.item() + \
                             vae_and_cls_loss.item()

            running_loss += ec_disc_loss.item() + \
                            ce_disc_loss.item() + \
                            vae_and_cls_loss.item()
...
for domain in targets_zh:
    ...
    opts = [ec_disc_opt, ce_disc_opt, vae_and_cls_opt]
