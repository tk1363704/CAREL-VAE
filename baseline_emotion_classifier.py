...
parser.add_argument('--self_iter', type=int, default=10, help='number of self train iteration')
parser.add_argument('--top_k', type=int, default=1, help='number of negative sentence in self training')
...
def print_training_info():
    ...
    print('training_iter-{} self-training_iter--{} negative_top_k--{}'.format(opt.training_iter, opt.self_iter,
                                                                              opt.top_k))
    # print('training_iter-{}'.format(opt.training_iter))
...
def generate_self_train_data(target_data, best_pred, K):
    ...
    for i in range(len(target_data.doc_id)):
        ...
        if sort_emo_prob:
            ...
            neg_emo_sen_index = int(sort_none_prob[-1][0].split(",")[0])
            neg_emo_index = random.randint(0, 5)
            self_y_emotion[i][neg_emo_sen_index][neg_emo_index] = 1

    print('# pos-emo doc:',count)
...
def train():
    ...
    empty = False
    num_iter = 0
    for i in range(opt.self_iter):
        self_best_model = load_ckp(os.path.join('ECPE_model/best_emotion_model', model_id + ".pt"), self_model)
        self_train_loader = generate_self_train_data(ECPE_data['self_train'], self_best_test_pred_emotion, opt.top_k)
        for j in range(opt.training_iter):
            start_time, step = time.time(), 1
